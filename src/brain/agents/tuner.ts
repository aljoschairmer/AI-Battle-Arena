...
export class TunerAgent extends Agent<TunerInput, PolicyPatch> {
  ...
  protected systemPrompt(): string {
    return [
      ...
      "- minTradeAdvantage (-1..0.4): how favourable a fight must look before we commit. Raise toward 0/+ if",
      "  we keep dying in fights we shouldn't take; lower if we're too passive and never engage.",
      "  CAUTION: values near 0.4 mean near-total combat refusal — if kills/round is dropping while you",
      "  raise this, you are in a passivity loss-spiral: LOWER it again.",
