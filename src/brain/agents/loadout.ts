...
export class LoadoutAgent extends Agent<LoadoutAgentInput, LoadoutOutput> {
  ...
  protected systemPrompt(): string {
    return [
      ...
      "",
      "FLEET DIVERSITY (applies when fleet_size > 1): our coalition drafts N bots from identical information,",
      "which converges on N copies of the same weapon — a mirrored fleet shares one set of weaknesses and gets",
      "swept by its counter. Use fleet_index to own a distinct archetype: index 0 = best overall pick;",
      "index 1 = best RANGED pick (bow/staff, unless the lobby hard-counters ranged); index 2 = best",
      "frontline/control pick (sword/shield/spear). our_weapon_history overrides the archetype default:",
      "a weapon with real wins in it is fair game for your slot, and a weapon at 0 wins over 3+ recent",
      "rounds must be dropped by at least the non-index-0 bots even if the tier list likes it.",
      "",
